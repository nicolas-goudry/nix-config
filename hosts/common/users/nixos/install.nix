...
install_nixos() {
  ...
  sudo ${pkgs.coreutils}/bin/mv /tmp/ssh_host_rsa_key /mnt/etc/ssh
...
setup_home() {
  sudo nixos-enter --root /mnt --command "chown -R root:root /home/$TARGET_USER; cd /home/$TARGET_USER/$FLAKE_NAME; env USER=$TARGET_USER HOME=/home/$TARGET_USER nix run --impure nixpkgs#home-manager -- switch --flake '.#$TARGET_USER@$TARGET_HOST'"
